...
<ResponsiveComponent>
{({size})=>{
  return size && size >=480?(<div className="flex items-center justify-center relative animate-spin-slow hover:pause">
...
<>
<div className="w-full px-2.5 xs:p-0 xs:w-max flex flex-col items-start xs:items-center justify-center relative space-y-4  group">
...
<div className="w-full px-2.5 xs:p-0 xs:w-max flex flex-col items-end xs:items-center justify-center relative space-y-4  group">
  {BtnList.slice(BtnList.length/2,BtnList.length).map((btn, index) => {
  

    return (
<NavButton labelDirection={'left'} key={btn.label} x={0} y={0} {...btn} />
    );
  })}
</div>
